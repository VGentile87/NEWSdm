...
void Make_SRIM::Loop()
{
  ...
  TTree * Tree_out = new TTree("tree","data");
  ...
  Tree_out->Branch("ev",&ev,"ev/F");
  Tree_out->Branch("xm",&xm,"xm/F");
  Tree_out->Branch("ym",&ym,"ym/F");
  Tree_out->Branch("zm",&zm,"zm/F");
  Tree_out->Branch("lenm_2d",&lenm_2d,"lenm_2d/F");
  Tree_out->Branch("lenm_3d",&lenm_3d,"lenm_3d/F");
  Tree_out->Branch("phim",&phim,"phim/F");
  Tree_out->Branch("them",&them,"them/F");
  Tree_out->Branch("srim_x",&srim_x,"srim_x/F");
  Tree_out->Branch("srim_y",&srim_y,"srim_y/F");
  Tree_out->Branch("srim_z",&srim_z,"srim_z/F");
  Tree_out->Branch("srim_len_2d",&srim_len_2d,"srim_len_2d/F");
  Tree_out->Branch("srim_len_3d",&srim_len_3d,"srim_len_3d/F");
  Tree_out->Branch("srim_phi",&srim_phi,"srim_phi/F");
  Tree_out->Branch("srim_the",&srim_the,"srim_the/F");
  Tree_out->Branch("kene",&kene,"kene/F");
  Tree_out->Branch("rene",&rene,"rene/F");
  
  const int ncr_per_um =20;// 20;//22;//0;  //5 per geant4
  const int nbin2=174;
  float lim_nbin1=ncr_per_um*30;//660;//502.5;
  float lim_nbin2=ncr_per_um*30;//660;//504.6;
  float zstep=50;
  int nlayers=20;//20;  //5 per geant4
  int min_gap=0;
  int z_min_gap=0;
  ...
  TH1F *hphi_srim = new TH1F("phi_srim","phi_srim",100,-pi/2.,pi/2.);
  TH1F *hthe_srim = new TH1F("the_srim","the_srim",100,-pi/2.,pi/2.);
  TH1F *h_phi_bar = new TH1F("phi_bar","phi_bar",100,-pi/2.,pi/2.);
  TH1F *h_phim = new TH1F("phim","phim",100,-pi/2.,pi/2.);
  TH1F *h_the = new TH1F("the","the",100,-pi/2.,pi/2.);
  TH1F *h_len = new TH1F("len","len",100,0.,600);
  TH1F *h_len_2 = new TH1F("len2","len2",600,0.,600);
  TH1F *h_len_3 = new TH1F("len3","len3",600,0.,600);
  TH1F *hsrim2 = new TH1F("srim2","srim2",600,0.,600);
  TH1F *hsrim3 = new TH1F("srim3","srim3",600,0.,600);
  TH1F *h_len_srim = new TH1F("len_srim","len_srim",600,0.,600);
  TH1F *h_nolen = new TH1F("no_len","no_len",100,0.,600);
  TH1F *h_sens_len = new TH1F("sens_len","sens_len",100,0.,600);

  TH3F *h3_1 = new TH3F("h3_1","h3_1",500,-250,250,500,-250,250,250,0,250);
  TH3F *h3_2 = new TH3F("h3_2","h3_2",500,-250,250,500,-250,250,250,0,250);

  TH2F *h2_1 = new TH2F("h2_1","h2_1",500,-250,250,500,-250,250);
  TH2F *h2_2 = new TH2F("h2_2","h2_2",500,-250,250,500,-250,250);

  TH2F *hdiff = new TH2F("hdiff","hdiff",250,0,250,250,0,250);
  ...
  TH1F *hzcr = new TH1F("zcr","zcr",50,-1000,1000);
  TH1F *hycr = new TH1F("ycr","ycr",50,-1000,1000);
  TH1F *hxcr = new TH1F("xcr","xcr",30,-100,1400);
  ...
  TProfile *hle = new TProfile("hle","hle",100,0,10,0,600);

  TProfile *h_length = new TProfile("len_prof","len_prof",100,0,600,0,600);

  TH2F *h_len_fil_3d = new TH2F("h_len_fil_vs3d","h_len_fil_vs_3d",250,0,250,250,0,250);
  TH1F *h_len_fil = new TH1F("h_len_fil","h_len_fil",250,0,250);
  ...
  for(int k=0;k<nlayers;k++){
    ...
    /// FRAME POSIZIONAMENTO CRISTALLO NELLA CELLA
    for(int i=0;i<ncr_per_um;i++){
      for(int j=0;j<ncr_per_um;j++){
  ...
  for(int s=0;s<nlayers;s++){
    /*c4[s] = new TCanvas(Form("cnv_%d",s),Form("cnv_%d",s),600,600);
    gr[s] = new TGraph();
    for(int j=0;j<z_pos.size();j++){
      if( j>=(ncr_plane*s) && j<(ncr_plane*(s+1)))gr[s]->SetPoint(j,z_pos.at(j),y_pos.at(j));	 
      if(j==z_pos.size()-1){
	c4[s]->cd();
	gr[s]->Draw("AP");      
      }          
    ...
    for(int j=0;j<z_pos.size();j++){
      delta_x=0;
      /*if( j>=(ncr_plane*s) && j<(ncr_plane*(s+1))){
	TArc *arc = new TArc(z_pos.at(j),y_pos.at(j),radius.at(j));
	...
	/*if(TMath::Abs(z_pos.at(j))<1000 && TMath::Abs(y_pos.at(j))<1000 && abs(x_pos.at(j)-s*zstep)<1)*/
	  x_pos.at(j) = x_pos.at(j) +  t2->Uniform(-15,0);
	...
	//	cout << "secondo layer " << j << " " << x_pos.at(j) << endl;
	  ...
	  for(int v=0;v<100;v++){
	    for(int i=0;/*start_id*/i<(ncr_plane*s);i++){
	      cr_dist = sqrt(pow(z_pos.at(j)-z_pos.at(i),2)+pow(y_pos.at(j)-y_pos.at(i),2)+pow((x_pos.at(j)-v)-x_pos.at(i),2));
  ...
  c44[0] = new TCanvas(Form("cnv2_%d",0),Form("cnv2_%d",0),600,600);
  gr2[0] = new TGraph();
  ...
  for(int i=0;i<x_pos.size();i++){
    ...
    for(int j=(i+1);j<x_pos.size();j++){
      ...
      if(i==0){
	if(TMath::Abs(z_pos.at(j))<1000 && y_pos.at(j)<-600 )gr2[0]->SetPoint(j,z_pos.at(j),x_pos.at(j));	 
	if(j==z_pos.size()-1){
	  c44[0]->cd();
	  gr2[0]->Draw("AP");      
	}
  ...
  */
  ...
  vector<float> y_hit;
  vector<float> z_hit;
  vector<float> trk_hit(6);

 
  TFile * f_in = new TFile("C30barshift.root","READ");
  TH1F * hbar = (TH1F*)f_in->Get("h");
  ...
  if (fChain == 0) return;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     y_hit.push_back(y);
     z_hit.push_back(z);
     if(rene==0 && index<100000 && kene==0){   /// alla fine della traccia
       tmp_dist_2=0;
       tmp_dist_3=0;
       tmp_first=-1;
       tmp_last=-1;
	 ...
	 for(int j=i;j<x_hit.size();j++){
	   len_3 = sqrt(pow(z_hit.at(j)-z_hit.at(i),2)+pow(y_hit.at(j)-y_hit.at(i),2)+pow(x_hit.at(j)-x_hit.at(i),2));
	  ...
	  tmp_last=j;
	  tmp_dist_2=len_2;
	  phim = atan2(z_hit.at(j)-z_hit.at(i),y_hit.at(j)-y_hit.at(i));	     
	  }
	  }
	  }*/
       ...
       // cout << ev << " " << x_hit.size() << " " << tmp_first << " " << tmp_last << endl;
       cout << ev << " " << x_hit.at(x_hit.size()-1) << " " << x_hit.at(0) << endl;
       //cout << ev << " " << atan2(z_hit.at(z_hit.size()-1)-z_hit.at(0),y_hit.at(y_hit.size()-1)-y_hit.at(0)) << " " << phim << endl;
       
       
       
       //cout << index << " " << z_hit.at(tmp_first) << " " << y_hit.at(tmp_first) << endl;
       //cout << tmp_first << endl;
       float dist_m1 = sqrt(pow(z_hit.at(tmp_first),2)+pow(y_hit.at(tmp_first),2)+pow(x_hit.at(tmp_first),2));
       float dist_m2 = sqrt(pow(z_hit.at(tmp_last),2)+pow(y_hit.at(tmp_last),2)+pow(x_hit.at(tmp_last),2));
       if(/*dist_m1<dist_m2*/x_hit.at(tmp_first)<x_hit.at(tmp_last)){
	 trk_hit[0]=z_hit.at(tmp_first);
	 trk_hit[1]=y_hit.at(tmp_first);
	 trk_hit[2]=x_hit.at(tmp_first);
	 trk_hit[3]=z_hit.at(tmp_last);
	 trk_hit[4]=y_hit.at(tmp_last);
	 trk_hit[5]=x_hit.at(tmp_last);
       }
       else{
	 trk_hit[0]=z_hit.at(tmp_last);
	 trk_hit[1]=y_hit.at(tmp_last);
	 trk_hit[2]=x_hit.at(tmp_last);
	 trk_hit[3]=z_hit.at(tmp_first);
	 trk_hit[4]=y_hit.at(tmp_first);
	 trk_hit[5]=x_hit.at(tmp_first);
       }
       
       
       //cout << ev << " " << tmp_dist_2 << " " << trk_hit[5] << " " << trk_hit[2] << " " << the <<  endl;
       //cout << ev << " " << srim_len << " "  << x_hit.at(x_hit.size()-1) << " " << x_hit.at(0) <<  " " << the_srim <<  endl;
       

       lenm_2d = tmp_dist_2;
       lenm_3d = tmp_dist_3;
       phim = atan((trk_hit[3]-trk_hit[0])/(trk_hit[4]-trk_hit[1]));
       them = atan(tmp_dist_2/(trk_hit[5]-trk_hit[2]));
       zm = trk_hit[3] - trk_hit[0];
       ym = trk_hit[4] - trk_hit[1];
       xm = trk_hit[5] - trk_hit[2];
       srim_z = z_hit.at(z_hit.size()-1)-z_hit.at(0);
       srim_y = y_hit.at(y_hit.size()-1)-y_hit.at(0);
       srim_x = x_hit.at(x_hit.size()-1)-x_hit.at(0);
       srim_len_2d = sqrt(pow(z_hit.at(z_hit.size()-1)-z_hit.at(0),2) + pow(y_hit.at(y_hit.size()-1)-y_hit.at(0),2));
       srim_len_3d = sqrt(pow(z_hit.at(z_hit.size()-1)-z_hit.at(0),2) + pow(y_hit.at(y_hit.size()-1)-y_hit.at(0),2) + pow(x_hit.at(x_hit.size()-1)-x_hit.at(0),2));
       srim_the = atan(srim_len_2d/(x_hit.at(x_hit.size()-1)-x_hit.at(0)));
       srim_phi = atan((z_hit.at(z_hit.size()-1)-z_hit.at(0))/(y_hit.at(y_hit.size()-1)-y_hit.at(0)));
       
       
       h3_1->Fill(srim_z,srim_y,srim_x);
       h3_2->Fill(trk_hit[3]-trk_hit[0],trk_hit[4]-trk_hit[1],trk_hit[5]-trk_hit[2]);
       h2_1->Fill(srim_z,srim_y,srim_x);
       h2_2->Fill(trk_hit[3]-trk_hit[0],trk_hit[4]-trk_hit[1],trk_hit[5]-trk_hit[2]);
       
       hsrim2->Fill(srim_len_2d);
       hsrim3->Fill(srim_len_3d);
       hphi_srim->Fill(srim_phi);
       hthe_srim->Fill(srim_the);
       ...
       z1 = t3->Uniform(-300,300);
       y1 = t2->Uniform(-300,300);
       x1 = t5->Uniform(100,700);
       z2 = z1 + trk_hit[3] - trk_hit[0];
       y2 = y1 + trk_hit[4] - trk_hit[1];
       x2 = x1 + trk_hit[5] - trk_hit[2];
       ...
       float delta_phi_fil = t6->Uniform(0,pi/2.);
       float bar_thr=7; //nm
       float dist_pl=0;
       //float dist_pl = len_srim*TMath::Sin(delta_phi_fil);
       do{
	 dist_pl = hbar->GetRandom();
	 //cout << jentry << " " << dist_pl << " " << len_srim+bar_thr << endl;
       }while(dist_pl>(len_srim+bar_thr));
       h_len_fil_3d->Fill(dist_pl,distAB);
       h_len_fil->Fill(dist_pl);
       
       len_proj = distAB;
       
       float diff = tmp_dist_2 - srim_len_2d;
       hdiff->Fill(srim_len_2d,diff);
       //cout << ev << " " << srim_len << " " << tmp_dist_2 << " " << diff << endl;
	 ...
	 if((distAC<radius.at(j) || distBC<radius.at(j)) || (dist_cr_hit < radius.at(j) && abs(proj1+proj2 - distAB)<1)){
	   ...
	   // cout << i << " " << j << " " << is_inside << " " << n_inside << " " << dist_cr_hit << " " << radius.at(j) <<   endl;
	   //cout << i << " " << j << " " << z_pos.at(j) << " " << y_pos.at(j) << " " << x_pos.at(j) <<  endl;
	   //cout << i << " " << j << " " << z1 << " " << y1 << " " << x1 << " " << z2 << " " << y2 << " " << x2 << endl;
	   

	   
	   //if((distAC<radius.at(j) && distBC>radius.at(j)) || (distAC>radius.at(j) && distBC<radius.at(j)))n_one_marginal_inside++; 
	   //if((distAC>radius.at(j) && distBC>radius.at(j)))n_zero_marginals_inside++;
	 ...
	 for(int k=0;k<tmp_cr.size();k++){
	   for(int j=(k+1);j<tmp_cr.size();j++){
	     ...
	     if(dist_cr>tmp_dist_cr){
	       ...
	       float r1 = radius.at(tmp_cr.at(k))+40;// + t1->Gaus(cryst_diam,cryst_sig);
	       float r2 = radius.at(tmp_cr.at(j))+40;// + t1->Gaus(cryst_diam,cryst_sig);
	       ...
	       float JP_pix=55; //nm
	       float first_gr = t1->Gaus(3.9,0.4)*JP_pix;
	       float second_gr = t2->Gaus(3.9,0.4)*JP_pix;
	       minor = first_gr + second_gr;
	       major = first_gr + second_gr + len_nit;
	 ...
	 two_grains++;	 
       ...
       if(abs(srim_the)<0.1 && tmp_first==0 && tmp_last==x_hit.size())cout << srim_len_2d << " " << len_2 << endl;
   ...
   h_len_fil_3d->Draw();
   TCanvas *c000 = new TCanvas("c000","c000",600,600);
   h_len_fil->Draw();
   ...
   h_len_nit->Scale(1./h_len_nit->Integral());
   TH1F *h_len_cum = (TH1F*)h_len_nit->GetCumulative();

   h_len_nit_cut->Scale(1./h_len_nit_cut->Integral());
   TH1F *h_len_cum_cut = (TH1F*)h_len_nit_cut->GetCumulative();

   TCanvas *cc0 = new TCanvas("cc0","cc0",600,600);
   h_len_cum_cut->Draw();

   h_len_2->Scale(1./h_len_2->Integral());
   TH1F *h_true_len_cum = (TH1F*)h_len_2->GetCumulative();

   h_len_srim->Scale(1./h_len_srim->Integral());
   TH1F *h_true_len_cum_cut = (TH1F*)h_len_srim->GetCumulative();
   ...
   h_phi_nit->Draw("");
   //h_phi_nit->Rebin(4);
   h_phi_nit->Scale(1./h_phi_nit->Integral());

   TCanvas *c10 = new TCanvas("c10","c10",600,600);
   h_phi_nit_gap->Draw("");
   //h_phi_nit_gap->Rebin(4);
   h_phi_nit_gap->Scale(1./h_phi_nit_gap->Integral());

   TCanvas *cc10 = new TCanvas("cc10","cc10",600,600);
   h_phi_nit_gap_len->Draw("");
   //h_phi_nit_gap->Rebin(4);
   h_phi_nit_gap_len->Scale(1./h_phi_nit_gap_len->Integral());

   TCanvas *c11 = new TCanvas("c11","c11",600,600);
   h_phi_nit_cut->Draw("");
   //h_phi_nit_cut->Rebin(4);
   h_phi_nit_cut->Scale(1./h_phi_nit_cut->Integral());
   
   TCanvas *c12 = new TCanvas("c12","c12",600,600);
   h_len_srim->Draw("HIST");
   h_len_nit_cut->Draw("sames && HIST");

   TCanvas *c13 = new TCanvas("c13","c13",600,600);
   h_true_len_cum_cut->Draw("");
   h_len_cum_cut->Draw("sames");

   TCanvas *c14 = new TCanvas("c14","c14",600,600);
   h_phi_nit_plasmon->Scale(1./h_phi_nit_plasmon->Integral());
   h_phi_nit_plasmon->Draw("HIST");
   ...
   TCanvas *c17 = new TCanvas("c17","c17",1200,600);
   c17->Divide(2,1);
   c17->cd(1);
   h3_1->Draw("");
   c17->cd(2);
