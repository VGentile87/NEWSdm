...
void Make_SRIM::Loop()
{
  ...
  while (1) {
    //for(int i=0;i<10;i++){
    in >> idg4  >>trk_hit[index][5] >>trk_hit[index][4] >> trk_hit[index][3] >> pdg;
    //cout << index << " " << idg4 << " " << pdg << endl;
  ...
  TH1F * hjpmin = (TH1F*)f_in->Get("htemp");
   ...
   for(int i=0;i<index;i++){
     ...
     int ntimes=0;
     do{
       z1 = t3->Uniform(-2000,-1500);
       y1 = t2->Uniform(-2000,-1500);
       x1 = t5->Uniform(250,750);
       z2 = z1 + trk_hit[i][4]*1000;// - trk_hit[i][0];
       y2 = y1 + trk_hit[i][5]*1000;// - trk_hit[i][1];
       x2 = x1 + trk_hit[i][3]*1000;// - trk_hit[i][2];
       //cout << i << " " << x1 << " " << x2 << endl;
       ntimes++;
	 ...
	 for(int k=0;k<tmp_cr.size();k++){
	   for(int j=(k+1);j<tmp_cr.size();j++){
	     ...
	     if(dist_cr>tmp_dist_cr){
	       ...
	       float tmp_minor = (first_gr + second_gr)/2.;// + t1->Uniform(0,JP_pix*sqrt(2));
	       float tmp_major = (first_gr/2. + second_gr/2. + len_nit);// + t2->Uniform(0,2*JP_pix*sqrt(2));;
