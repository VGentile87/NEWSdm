...
if [ ! -f $file_name ]; then
    ...
    if [ $varname = '1' ]; then
	cp  $DM_ROOT_Dropbox/src/macros/start_v2.C .
...
EOC
	...
	cp $DM_ROOT_Dropbox/src/macros/myData_v5.C .
	sed -n "36, 40p" myData.h > tmp_file
	sed -i 27rtmp_file myData_v5.C
	if [ $var_im = '2' ]; then
	    echo "QUI CI ARRIVO"
...
EOC
	...
	cp $DM_ROOT_Dropbox/src/macros/run_gif.C .
	cp $DM_ROOT_Dropbox/src/macros/dgr.C .
