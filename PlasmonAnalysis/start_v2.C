...
void start_v2(){
  ...
  pPath = getenv ("DM_ROOT_Dropbox");
  gROOT->ProcessLine("gSystem->Load(\"libDMRoot\")");
  gROOT->ProcessLine("Vdmr->MakeClass(\"myData\")");
  gSystem->Exec("rm myData.C");
  gSystem->Exec("cp $DM_ROOT_Dropbox/src/macros/myData_v5.C .");
  gSystem->Exec("cp $DM_ROOT_Dropbox/src/macros/DMPlsAnalyzer/DMPlsDefinitions.h .");
  gSystem->Exec("cp $DM_ROOT_Dropbox/src/macros/settings.mac .");
  //gSystem->Exec("cp $DM_ROOT/src/macros/run_gif.C .");
  gSystem->Exec("cp $DM_ROOT_Dropbox/src/macros/node.sh .");
