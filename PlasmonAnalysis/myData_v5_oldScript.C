...
Double_t eMTChi2=-1;
Double_t eMTBrDif=-1;
Double_t eMTNpxDif=-1;
Double_t eMTZDif=-1;
////////////////////////////////

Double_t eClFitMin=0;
Double_t eClFitMaj=0;
Double_t eClFitEll=0;
Double_t eClFitPhi=0;
Double_t eClFitx=0;
Double_t eClFity=0;
Double_t eSetFitBar=0;
Double_t eSetFitPhi=0;
Double_t eSetFitx=0;
Double_t eSetFity=0;
Double_t eSetFitPhiMean=0;
...
/*
std::vector <std::vector<double> > cl_fit_min;
std::vector <std::vector<double> > cl_fit_maj;
std::vector <std::vector<double> > cl_fit_phi;
std::vector <std::vector<double> > cl_fit_ell;
std::vector <std::vector<double> > cl_fit_x;
std::vector <std::vector<double> > cl_fit_y;
std::vector<double> gr_fit_x_mean;
std::vector<double> gr_fit_y_mean;
std::vector<double> gr_fit_phi_mean;
std::vector<double> gr_max_dist_fit;
std::vector<double> phi_set_fit;
*/
...
void myData::Loop()
{
  ...
  /*
  cl_fit_min.clear();
  cl_fit_min.resize(kMaxgr, std::vector<double>(kMaxgr));
  cl_fit_maj.clear();
  cl_fit_maj.resize(kMaxgr, std::vector<double>(kMaxgr));
  cl_fit_phi.clear();
  cl_fit_phi.resize(kMaxgr, std::vector<double>(kMaxgr));
  cl_fit_ell.clear();
  cl_fit_ell.resize(kMaxgr, std::vector<double>(kMaxgr));
  cl_fit_x.clear();
  cl_fit_x.resize(kMaxgr, std::vector<double>(kMaxgr));
  cl_fit_y.clear();
  cl_fit_y.resize(kMaxgr, std::vector<double>(kMaxgr));
  gr_fit_x_mean.clear();
  gr_fit_y_mean.clear();
  gr_fit_phi_mean.clear();
  gr_max_dist_fit.clear();
  phi_set_fit.clear();
*/
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) { // su tutte le View
    ...
    if((ncl<maxcl || jentry==cut_view) /*&& flag==0*/){   /// cut per view troppo sporche (maxcl va settato)   evita i CRASH
      ...
      std::vector<double> cl_pos(2);
      std::fill(cl_pos.begin(), cl_pos.end(), 0);
	...
	//dmpl.encoder_check(jn,fr_x[cl_ifr[jn]],fr_y[cl_ifr[jn]],fr_x[cl_ifr[0]],fr_y[cl_ifr[0]],cl_x[jn],cl_y[jn]);
	//dmpl.encoder_check(jn);
	...
	int tmp_mt_id=-1;
        int tmp_br_mt=-1;
	Double_t *br_mt = new Double_t[mt_ngr[kn]];
	Double_t *npx_mt = new Double_t[mt_ngr[kn]];
	Double_t *x_mt = new Double_t[mt_ngr[kn]];
	Double_t *y_mt = new Double_t[mt_ngr[kn]];
	Double_t *z_mt = new Double_t[mt_ngr[kn]];
	for(int in=0;in<gr_;in++){
	  if(gr_imt[in]==kn){
	    ...
	    imt++;
	    /// correzione microtracce
	    //if((mt_ngr[kn]-1)/mt_len[kn]>2.45 && gr_imt[in]==kn){
	    // }
	  }
	}
	...
	mt_dif_npx[tmp_mt_id]=TMath::MaxElement(imt,npx_mt) - TMath::MinElement(imt,npx_mt); // differenza tra area  massima e minima dei grani in una microtraccia
	mt_dif_br[tmp_mt_id]=TMath::MaxElement(imt,br_mt) - TMath::MinElement(imt,br_mt); // differenza tra luminosità  massima e minima dei grani in una microtraccia
	mt_dif_z[tmp_mt_id]=TMath::MaxElement(imt,z_mt) - TMath::MinElement(imt,z_mt); // 
	//cout << "microtracce " << kn << " " << imt << " " << tmp_mt_id << " " << mt_dif_br[tmp_mt_id] << endl;
	delete [] br_mt;
	delete [] npx_mt;
	delete [] x_mt;
	delete [] y_mt;
